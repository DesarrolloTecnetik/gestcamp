                    ...
                    if ($this->processed && $image_src) {
                        ...
                        if ($this->image_watermark!='' && file_exists($this->image_watermark)) {
                            ...
                            if ($watermark_type == IMAGETYPE_GIF) {
                                ...
                                if(!$this->function_enabled('imagecreatefromjpeg')) { $this->error = $this->translate('watermark_no_create_support', array('JPEG')); } else {

                                    $filter = @imagecreatefromjpeg($this->image_watermark);
                                    if (!$filter) { $this->error = $this->translate('watermark_create_error', array('JPEG')); } else { $this->log .= '&nbsp;&nbsp;&nbsp;&nbsp;watermark source image is JPEG<br />'; $watermark_checked = true; }
